...
import { GLP1_ACCORDION } from '@/components/Products/Single/ConstantsSingle';
...
export async function generateMetadata(): Promise<Metadata> {
  return {
    title: 'GLP-1 Weight Loss Plans – Personalized Telehealth Solutions | LumiMeds',
    description:
      "Choose from LumiMeds' GLP-1 weight loss plans tailored to your goals. Includes telehealth consultations, prescriptions, and ongoing support.",
    robots:
      process.env.NEXT_PUBLIC_ALLOW_INDEXING === 'true'
        ? 'index, follow, max-image-preview:large, max-snippet:-1, max-video-preview:-1'
        : 'noindex, nofollow',
    alternates: {
      canonical: 'https://www.lumimeds.com/products/glp-1-plans',
    },
    openGraph: {
      title: 'GLP-1 Weight Loss Plans – Personalized Telehealth Solutions | LumiMeds',
      description:
        "Choose from LumiMeds' GLP-1 weight loss plans tailored to your goals. Includes telehealth consultations, prescriptions, and ongoing support.",
      type: 'website',
      url: 'https://www.lumimeds.com/products/glp-1-plans',
    },
  };
}

export default async function GLP1PlansPage() {
  const products = await fetchProducts({
    keys: [
      { name: 'glp_1_plans', categories: ['weight_loss_glp_1_injection_recurring'], planTypeSort: PlanType.RECURRING },
    ],
  });

  const product = products.glp_1_plans;
  const accordionData = GLP1_ACCORDION;
  const accordionTitle = 'GLP-1 Injections';
          ...
          <div className='row bg-3060fe-radius-24'>
            ...
            >
              ...
              <p id='glp-1-description'>
                GLP-1 medications mimic the GLP-1 hormone the body produces immediately after food intake. When you eat,
                your digestive system releases this hormone to stimulate insulin production and balance blood sugar
                levels. GLP-1 medications mirror the effects of the naturally occurring hormone, which also reduces
                appetite and promotes feelings of fullness. These effects work together to help patients lose weight.
              </p>
            </div>
          </div>
        </div>
