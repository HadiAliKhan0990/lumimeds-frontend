import { PROGRAM_SUMMARY_ITEMS } from '@/constants/checkoutProgramSummary';
      ...
      {PROGRAM_SUMMARY_ITEMS.map((item) => {
        ...
        return (
          <div key={item.id} className='tw-flex tw-items-center tw-gap-2 md:tw-gap-4'>
            ...
            <span className='tw-text-[0.74rem] sm:tw-text-sm tw-font-medium tw-text-gray-900 tw-flex-1'>{item.label}</span>
          </div>
        );
      })}
    </div>
