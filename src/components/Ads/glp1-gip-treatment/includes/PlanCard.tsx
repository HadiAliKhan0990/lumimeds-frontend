...
export default function PlanCard({ product, footnote, imageAlt, fallbackImage }: Readonly<PlanCardProps>) {
  ...
  return (
    <div className='product-card plan-card rounded-4 p-4 p-md-5 h-100 d-flex flex-column'>
      <div className='card-media text-center mb-3 mb-md-4'>
        {config.imageSrc && (
          <Image
            src={config.imageSrc}
      ...
      <div className='card-body p-0'>
        <div className='plan-pricing mb-3'>
          ...
          {config.isValue && (
            <div className='flex flex-row text-center'>
              ...
              <span>${getRoundedPrice(config.upfront)} upfront</span>
            </div>
          )}
        </div>
        ...
        <div className='mt-3 text-center text-lg-start'>
          <button
            ...
            data-tracking-id={`product-card-glp1-gip-${product.id || product.name?.toLowerCase().replace(/\s+/g, '-')}`}
          >
