...
import KlarnaIcon from '@/assets/ads/google-shopping/klarna-icon.svg';
...
export default function Hero({ product }: HeroProps) {
  ...
  const badges = [
    ...
    ...(isTrySubdomain ? [] : [{ icon: KlarnaIcon, alt: 'Klarna' }]),
  ];

    // Construct heading text conditionally based on try subdomain
    const tirzepatideHeading = isTrySubdomain 
    ? (
        <>
          {`${GLP1_GIP_PRODUCT_NAME.replace('Compounded ', '')} (Compounded)`}
          <br />
          {GLP1_GIP_LABEL.replace(/[()]/g, '')}
        </>
      )
    : 'Your First 90 Days of Clinically Guided GLP-1/GIP Care';
  ...
  return (
    <div className='tw-flex md:tw-flex-row tw-flex-col lg:tw-gap-[6.25vw] tw-gap-6 tw-justify-end'>
      <div className='tw-flex tw-flex-col tw-justify-center tw-flex-1 lg:tw-pl-[6.25vw] md:tw-pl-5 md:tw-pr-0 tw-px-4'>
        ...
        <div className='lg:tw-max-w-[37.708vw] tw-w-full'>
          <p className='tw-font-lato tw-text-[#737D97] md:tw-text-base tw-text-xs tw-leading-[120%] tw-text-center md:tw-m-0 tw-mb-6 md:tw-max-w-full tw-max-w-[372px] tw-mx-auto'>
            A clinically guided 3-month starter pack featuring Compounded Tirzepatide (GLP-1/GIP)
            {isTrySubdomain ? null : <span data-injections-text="true"> Injections</span>} for those
            beginning treatment.
          </p>
        </div>
        <div className='tw-block md:tw-hidden tw-max-w-[408px] tw-h-[280px] tw-overflow-hidden tw-rounded-2xl'>
          <div
        ...
        <div className='tw-max-[35.417vw] md:tw-mt-[3.472vw] tw-mt-10 md:tw-mb-3 tw-mb-6'>
          ...
          >
            ...
            <span>{isTrySubdomain ? 'Add to Cart' : 'See If You Qualify'}</span>
          </button>
        ...
        <div className='tw-flex tw-items-center tw-justify-evenly lg:tw-gap-[0.833vw] tw-gap-6 tw-mx-1.389vw lg:tw-mt-[4.167vw] md:tw-my-5 tw-mt-6 tw-mb-12'>
          {badges.map(({ icon, alt }) => (
            <div 
              ...
              {...(alt === 'Klarna' ? { 'data-klarna-icon': 'true' } : {})}
            >
