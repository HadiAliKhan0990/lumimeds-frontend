    ...
    .map((product, index) => {
      ...
      const priceNote =
        isRecurring && intervalCount > 1
          ...
          : product.planType === PlanType.ONE_TIME
          ? `${toCurrency(amount)} upfront - ${toCurrency(amount / Math.max(intervalCount, 1))}/mo equivalent`
          : product.featureText || undefined;

      const displayPerMonth =
        product.planType === PlanType.ONE_TIME && intervalCount > 1
          ? amount / intervalCount
          : isRecurring && intervalCount > 1
          ? product.dividedAmount
          : amount;
      ...
      return {
        ...
        subtitle,
        priceDisplay: String(getRoundedPrice(displayPerMonth)),
        priceSuffix,
        priceNote,
...
export default function PlanCard({ data, renderImage }: Readonly<Props>) {
  const category = data?.glp_1_gip_plans;
        ...
        >
          ...
          <div className='tw-flex tw-w-full lg:tw-w-[55%] tw-flex-col tw-gap-3 lg:tw-gap-4 xl:tw-gap-6 tw-text-left'>
            <div className='tw-space-y-2'>
              {plan.badge && (
                <span className='tw-text-[#FFE770] tw-text-[20px] lg:tw-text-[24px] xl:tw-text-[31px] tw-leading-[32px] lg:tw-leading-[44px] xl:tw-leading-[56px] tw-font-secondary tw-font-bold'>
                  {plan.badge} Plan
                </span>
              )}
              {/* {plan.title && (
                <h4 className='tw-text-white tw-text-[28px] md:tw-text-[32px] tw-font-semibold tw-leading-[120%]'>
                  {plan.title}
                </h4>
              )}
              {plan.subtitle && (
                <p className='tw-text-[#FBE2CA] tw-text-[16px] md:tw-text-[18px] tw-font-medium tw-leading-[150%]'>
                  {plan.subtitle}
                </p>
              )} */}
            </div>
            ...
            <div className='tw-space-y-2'>
              <div className='tw-flex tw-items-end'>
                <span className='tw-text-[30px] lg:tw-text-[40px] xl:tw-text-[50px] tw-leading-[80px] lg:tw-leading-[100px] xl:tw-leading-[133px] tw-font-bold tw-text-white tw-font-secondary'>
                  $
                </span>
                <span className='tw-text-[60px] lg:tw-text-[80px] xl:tw-text-[100px] tw-font-bold tw-leading-[80px] lg:tw-leading-[100px] xl:tw-leading-[133px] tw-text-white tw-font-secondary tw-pb-3 lg:tw-pb-3.5 xl:tw-pb-4'>
                  {plan.priceDisplay}
                </span>
                <span className='tw-text-[20px] lg:tw-text-[30px] xl:tw-text-[40px] tw-leading-[80px] lg:tw-leading-[100px] xl:tw-leading-[133px] tw-font-bold tw-text-white tw-font-secondary'>
                  /mo
                </span>
                {/* {plan.priceSuffix && (
                  <span className='tw-text-[22px] md:tw-text-[26px] tw-font-semibold tw-text-[#FFD461]'>
                    {plan.priceSuffix}
                  </span>
                )} */}
              </div>
              {/* {plan.priceNote && (
                <p className='tw-text-[16px] md:tw-text-[18px] tw-text-[#FBE2CA] tw-leading-[150%]'>{plan.priceNote}</p>
              )} */}
            </div>
            ...
            {plan.bullets.length > 0 && (
              <ul className='tw-flex tw-flex-col tw-gap-2 tw-px-0 tw-mb-0'>
                {plan.bullets.map((bullet, index) => (
                  ...
                  >
                    <span className='tw-text-white tw-text-[28px] lg:tw-text-[32px] xl:tw-text-[36px] tw-leading-[16px]'>
                      •
                    </span>
                    <span className='tw-text-white tw-text-[15px] lg:tw-text-[17px] xl:tw-text-[19px] tw-font-medium tw-leading-[24px] xl:tw-leading-[32px] tw-letter-spacing-[-0.19px]'>
                      {bullet}
                    </span>
                  </li>
                ))}
            ...
            <SurveyGetStartedButton
              ...
              className='tw-inline-flex tw-items-center tw-justify-center tw-gap-2 !tw-text-center !tw-rounded-[26px] !tw-bg-[#FFE770] !tw-text-[#6C361D] !tw-text-[16px] md:!tw-text-[19px] lg:!tw-text-[22px] xl:!tw-text-[27px] !tw-font-poppins !tw-leading-[133%] !tw-font-semibold !tw-px-6 md:!tw-px-8 lg:!tw-px-12 xl:!tw-px-16 !tw-py-1 md:!tw-py-1.5 lg:!tw-py-2 xl:!tw-py-2.5 hover:tw-bg-[#FFE18A] disabled:tw-opacity-60 disabled:tw-cursor-not-allowed max-lg:tw-mt-2'
              style={{ backgroundColor: '#FFE770', color: '#6C361D', borderRadius: '26px' }}
            >
