...
interface TestimonialsSectionProps extends React.HTMLAttributes<HTMLDivElement> {
...
const TestimonialsSection: React.FC<TestimonialsSectionProps> = ({
  ...
  */
  ...
  const sectionClassName = ['testimonials-section', props.className].filter(Boolean).join(' ').trim();

  return (
    <section
      className={sectionClassName}
      style={{ backgroundColor, paddingTop, paddingBottom: paddingTop, ...props.style }}
      {...props}
    >
      <div className='container-fluid'>
        <div className='testimonials-container tw-max-w-[1200px] mx-auto'>
          {showTitle && <h2 className={`${titleTextClassName} tw-text-center tw-mb-10`}>{t('title')}</h2>}
