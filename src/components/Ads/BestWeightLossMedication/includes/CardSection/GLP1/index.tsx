...
// Import GLP1 specific image or use product image
...
export default function GLP1CardSection({ data }: Readonly<Props>) {
  ...
  // Get GLP-1 products from the data
  const glp1Products = data?.glp_1_plans?.products || [];

  // Filter products by plan tier
  const starterProduct = glp1Products.find((product) => product.metadata?.planTier === 'starter');
  const valueProduct = glp1Products.find((product) => product.metadata?.planTier === 'value');

  // If no value product found, use the first available product as value
  const displayValueProduct = valueProduct || (glp1Products.length > 1 ? glp1Products[1] : null);
  ...
  return (
    <div className={styles.glp1CardSection}>
            ...
            <div className={styles.frameContainer}>
              <div className={styles.compoundedTirzepatideGlp1Parent}>
                <b className={styles.compoundedTirzepatideGlp1Container}>
                  <p className={styles.compoundedTirzepatide}>{GLP1_PRODUCT_NAME}</p>
                  <p className={styles.compoundedTirzepatide}>{GLP1_LABEL} Injections</p>
                </b>
              ...
              <div className={styles.monthStarterSubscriptionForParent}>
                <div className={styles.monthStarterSubscriptionContainer}>
                  <span className={styles.monthStarterSubscriptionContainer2}>
                    <p className={`${styles.compoundedTirzepatide} tw-line-height-[0.5rem]}`}>
        ...
        <div className={`col-12 col-lg-5 order-1 order-xl-2 ${styles.rightSide}`}>
          <div className={styles.vialGipglp2}>
            <Image
              ...
              height={400}
              sizes='100vw'
              alt={`Lumimeds Compounded ${GLP1_PRODUCT_NAME} injection vial`}
            />
          </div>
        </div>
      </div>
