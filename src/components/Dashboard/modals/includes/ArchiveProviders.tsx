...
export function ArchiveProviders() {
  ...
  useEffect(() => {
    const handleProviderToArchive = (event?: CustomEvent) => {
      if (event?.detail?.provider) setSingleProvider(event?.detail?.provider);
    };

    window.addEventListener('singleProviderToArchive', handleProviderToArchive as EventListener);

    return () => {
      window.removeEventListener('singleProviderToArchive', handleProviderToArchive as EventListener);
    };
  }, []);
  ...
  const handleConfirm = async () => {
    try {
      await archiveProviders({
        providerIds: selectedRows,
        isArchived: true,
      }).unwrap();

      toast.success(
        selectedRows?.length > 1
          ? `${selectedRows?.length} Providers have been archived successfully`
          : 'Provider has been archived successfully'
      );

      // Emit custom event to notify providers page to refetch data
      const event = new CustomEvent('providersArchived', {
      ...
      console.log(error);
      toast.error(
        selectedRows?.length > 1 ? `${selectedRows?.length} Providers failed to archive` : 'Provider failed to archive'
      );
    }
  };
