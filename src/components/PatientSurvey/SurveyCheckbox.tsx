import { SurveyQuestion } from '@/store/slices/surveyQuestionSlice';
import { FaRegCircle } from 'react-icons/fa6';
import { FaRegCheckCircle } from 'react-icons/fa';
import { PatientSurveyAnswerType } from '@/lib/types';
import { usePathname } from 'next/navigation';
import { ROUTES } from '@/constants';
import { motion } from 'framer-motion';
import { placeholderConstants } from './PlaceHolderConstants';

interface Props {
  question: SurveyQuestion;
  answers: PatientSurveyAnswerType[];
  setAnswers: (ag: PatientSurveyAnswerType[]) => void;
}

export default function SurveyCheckbox({ question, answers, setAnswers }: Props) {
  const pathname = usePathname();

  const answer = answers.find((answer) => answer.questionId === question.id)?.answer as string;
  const otherValue = answers.find((answer) => answer.questionId === question.id)?.otherText;

  const handleClick = (option: string) => {
    const isSelected = answer === option;
    const isRequired = question.isRequired ?? false;

    const _answers = answers.map((item) => {
      if (item.questionId !== question.id) return item;

      if (isSelected) {
        return {
          ...item,
          answer: undefined,
          otherText: undefined,
          isValid: !isRequired,
        };
      }

      return {
        ...item,
        questionId: question.id,
        answer: option,
        otherText: option.toLowerCase() === 'other' ? item.otherText : undefined,
        isValid: true,
      };
    });

    setAnswers(_answers);
  };

  function handleChange(e: React.ChangeEvent<HTMLInputElement>) {
    const value = e.target.value.slice(0, 100);

    const _answers = answers.map((answer) => {
      if (answer.questionId === question.id)
        return {
          questionId: question.id,
          answer: answer.answer,
          isValid: true,
          otherText: value,
        };
      return answer;
    });
    setAnswers(_answers);
  }

  // Find matching placeholder based on question text
  const matchingPlaceholder = placeholderConstants.find(
    (item) => item.question.toLowerCase() === question.questionText?.toLowerCase()
  )?.placeholder || 'Please specify';

  return question.options?.map((option, i) => {
    const isSelected = answer === option;
    const isOtherOption =
      (option.toLowerCase().includes('other') && !option.toLowerCase().includes('another')) ||
      option.toLowerCase().includes('please list');
    const shouldShowInput =
      isSelected &&
      isOtherOption &&
      !pathname.includes(ROUTES.PATIENT_SURVEY) &&
      !question.questionText?.toLowerCase().includes('gender');

    return (
      <div key={i}>
        <button
          style={{ border: `1px solid ${isSelected ? 'transparent' : 'black'}` }}
          onClick={() => handleClick(option)}
          className={`tw-rounded border tw-cursor-pointer tw-px-3 tw-py-2 md:tw-p-4 tw-text-sm md:tw-text-base tw-flex tw-select-none tw-items-center tw-justify-start tw-gap-x-4 tw-w-full tw-bg-${
            isSelected ? 'primary' : 'white'
          }`}
        >
          {isSelected ? (
            <FaRegCheckCircle className='flex-shrink-0' color='white' />
          ) : (
            <FaRegCircle className='flex-shrink-0' />
          )}
          <span className={`tw-text-sm md:tw-text-base text-start tw-text-${isSelected ? 'white' : 'black'}`}>
            {option}
          </span>
        </button>

        {shouldShowInput && (
          <motion.div
            initial={{ opacity: 0, height: 0 }}
            animate={{ opacity: 1, height: 'auto' }}
            transition={{ duration: 0.3, ease: 'easeInOut' }}
            className='tw-mt-4'
          >
            <input
              type='text'
              value={otherValue}
              onChange={handleChange}
              maxLength={100}
              placeholder={isOtherOption ? matchingPlaceholder : ''}
              className='form-control dark-input border-black rounded-1'
            />
          </motion.div>
        )}
      </div>
    );
  });
}
